...
public class DeviceProgressReportAction extends ReportAction {
	...
	private Set<SnapshotWindow> snapshots;
	...
	@ConversationScoped(value = "deviceProgressDataGridState")
	private DataGridState<?> reportGrid;

	@CoreSecured({ CorePermissionCodes.READY_CUSTOMER_PROGESS_DEVICE_RPT })
	@Action(value = "deviceProgress", results = { @Result(name = "report", location = "/net/techreadiness/plugin/action/reports/device/progress/report.jsp") })
	public String deviceProgress() throws Exception {
		getData();
		buildBreadcrumbs(currentOrg, consortium, "deviceProgress");
		return "report";
	}
	...
	private void getData() throws Exception {
		...
		snapshots = Sets.newTreeSet(new SnapshotComparator());
		snapshots.addAll(snapshotService.findActiveSnapshots(context, consortiumScope.getScopeId()));
	...
	protected ReportExport getReportExport(ExportType type) throws Exception {
		...
		for (SnapshotWindow snapshot : snapshots) {
			...
			columnKeys.add("devicePassingPercent_" + snapshot.getId().toString());
		...
		case pdf: {
			ReportExportPdf pdf = new ReportExportPdf.Builder(columnLabels, columnKeys, useConditionalStyles,
					...
					.title("Progress Report - Device Indicators")
					.subtitle(
							(currentOrg.getOrgTypeName().equals("Readiness") ? "" : currentOrg.getName() + " - ")
									+ "% Devices Meet the " + getRequirements() + " Requirements")
					.consortium(consortium.toString()).aggregationMessage(getAsOfDate() + " - Reports are updated hourly")
					.build();
	...
	public Collection<SnapshotWindow> getSnapshots() {
		return snapshots;
	}
