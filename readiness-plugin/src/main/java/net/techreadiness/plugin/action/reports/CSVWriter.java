...
public class CSVWriter {
	...
	public CSVWriter(Writer writer) {
		this(writer, DEFAULT_DELIMITER, DEFAULT_COMMENT);
	}
	 ...
	 *            writer to output to
	 * @param delimiter
	 *            field delimiter character to use
	 */
	public CSVWriter(Writer writer, char delimiter) {
		this(writer, delimiter, DEFAULT_COMMENT);
	}
	...
	public void writeCommentLine(final String comment) throws IOException {
		// newlines output without modification in the comment will result in an
		// invalid CSV line
	...
	public void writeLine(final String[] values) throws IOException {
		for (int ii = 0; ii < values.length; ii++) {
			escapeCSV(writer, values[ii], delimiter);
	...
	private void writeLine(final Collection<String> values) throws IOException {
		...
		for (String value : values) {
			escapeCSV(writer, value, delimiter);
			if (++ii < values.size()) {
				writer.append(delimiter);
			}
		}
	...
	public static String escapeCSV(final String text) {
		return escapeCSV(text, DEFAULT_DELIMITER);
	}
	...
	public static String escapeCSV(final String text, final char delimiter) {
		try {
			...
			throw new RuntimeException("escapeCSV error", e); // should not
																// happen with a
																// StringWriter
		}
	}
	 ...
	 *            text string to escape
	 * @param writer
	 *            writer to send the CSV output to
	 * @param delimiter
