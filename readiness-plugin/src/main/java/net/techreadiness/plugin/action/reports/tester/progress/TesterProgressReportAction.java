package net.techreadiness.plugin.action.reports.tester.progress;
...
public class TesterProgressReportAction extends ReportAction {
	...
	@Inject
	@Qualifier("TesterProgressItemProvider")
	private ReportItemProvider reportItemProvider;

	@ConversationScoped(value = "testerProgressDataGridState")
	private DataGridState<?> reportGrid;

	@CoreSecured({ CorePermissionCodes.READY_CUSTOMER_PROGESS_NETWORK_RPT })
	@Action(value = "testerProgress", results = { @Result(name = "report", location = "/net/techreadiness/plugin/action/reports/tester/progress/report.jsp") })
	public String testerProgress() throws Exception {
		getData();
		buildBreadcrumbs(currentOrg, consortium, "testerProgress");
		return "report";
	}
	...
	protected ReportExport getReportExport(ExportType type) throws Exception {
		...
		for (SnapshotWindow snapshot : snapshots) {
			...
			columnKeys.add("testTakerPercentStudentsTestable_" + snapshot.getId().toString());
		...
		case pdf: {
			...
			if (currentOrg.getOrgTypeName().equals("School")) {
				subtitle.append("<b>")
						.append(currentOrg.getOrgTypeName().equals("Readiness") ? consortium.toString() : currentOrg
								.getName()).append(" - ");
				subtitle.append("% of Eligible Test-Takers can be Tested on Existing Devices, based on ")
			...
			ReportExportPdf pdf = new ReportExportPdf.Builder(columnLabels, columnKeys, useConditionalStyles,
					reportItemProvider.export(type)).title("Progress Report - Device to Test-Taker Indicators")
