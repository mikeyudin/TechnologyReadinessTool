...
<table class="report-layout">
	<tr>
		<td class="left">
			<c:if test="${ dualConsortium }">
				...
				<div class="report-toggle">
					<s:form action="networkProgress" id="consortiumForm">
						<c:choose>
		...
		<td class="center">
			<div class="report-header">Progress Report - Network Indicators</div>
			...
			<c:choose>
				<c:when test="${ currentOrg.orgTypeName == 'Readiness' }">
					<div class="report-org-text">
						...
						<div class="report-label2">% of Students can be Tested in the Window, based on ${ requirements } Requirements <img src="${pageContext.request.contextPath}/static/images/moreinfo-reverse.png" title="# Possible Test Starts / # Test Starts Needed"/></div><div style="clear: both;"></div>
					</div>
				</c:when>
				<c:when test="${ currentOrg.orgTypeName == 'School' }">
					<div class="report-org-text">
						<div class="report-label">${ currentOrgName } - </div>
						<div class="report-label2">% Schools having Sufficient Infrastructure to Carry the Data Traffic for this Assessment, based on ${ requirements } Requirements <img src="${pageContext.request.contextPath}/static/images/moreinfo-reverse.png" title="Max possible test starts in window/ total number of test starts needed per school"/></div><div style="clear: both;"></div>
					</div>
				</c:when>
				<c:otherwise>
					<div class="report-org-text">
		...
		<td class="right">
			...
			<div class="report-legend">
				...
				<div class="legend-row">
					<div class="report-label"><ui:flyout text="TBD" title="Why are some reports showing TBD as a value?" namespace="/reports/network" action="TBD" reset="true" /></div>
				</div>
...
<s:form action="networkProgress" id="selectOrg">
	<s:hidden name="viewBy"/>
	<s:hidden name="consortium"/>
...
<div style="clear: both;"></div>
<s:form>
	<ui:datagrid itemProvider="${reportItemProvider}" value="reportGrid" var="network" selectable="false" rowValue="network.orgCode" columnSelectable="true" >
		<ui:dataGridHeader>
		...
		<s:iterator value="snapshots" var="snapshot">
			<s:set var="snapshotId">networkPercentStudentsTestable_${ snapshot.snapshotWindowId }</s:set>
			<s:set var="snapshotName">${ snapshot.name == 'default' ? 'Current Window' : snapshot.name }</s:set>
			<fmt:formatDate value="${ snapshot.executeDate }" pattern="MM/dd/yyyy hh:mm:ss a" var="executeDate" timeZone="US/Central"/>
			<s:url var="requirementsUrl" namespace="/reports/network" action="minRequirements">
			...
			<s:url var="networkAssessmentUrl" namespace="/reports/network" action="networkAssessment">
				<s:param name="snapshotName" value="#snapshot.name"/>
				<s:param name="snapshotScopeId" value="#snapshot.scope.id"/>
			...
			<ui:dataGridColumn code="${snapshotId}" manageColumnsText="${snapshot.name == 'default' ? 'Current Window' : snapshot.name}" name="${snapshot.name == 'default' ? 'Current Window' : snapshot.name}<a class='entity-name' title='Minimum / Recommended Requirements - ${ executeDate }' href='${ requirementsUrl }'><img style='margin-left: 4px' src='${pageContext.request.contextPath}/static/images/magnifying-glass-white.png'/></a><br>${ executeDate }<a class='#report-desc' title='View of Network Indicators Report as of ${ executeDate }' href='${ networkAssessmentUrl }'><img style='margin-left: 3px' src='${pageContext.request.contextPath}/static/images/external-link-white.gif'/></a>">
				<s:if test="%{ network[#snapshotId] == 'TBD' }">
					<span class="tbd">TBD</span>
				</s:if>
				<s:else>
				   <div class="report-datagrid-color-icon">
						<div class="color <s:property value="getPercentColor(network[#snapshotId])"/>"></div>
						<div class="completion-label"><s:property value="network[#snapshotId]"/></div>
					</div>
				</s:else>
			</ui:dataGridColumn>
		</s:iterator>
