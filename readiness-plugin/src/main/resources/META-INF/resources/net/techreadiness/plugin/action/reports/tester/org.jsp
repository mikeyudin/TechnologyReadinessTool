...
<table class="report-layout">
	<tr>
		...
		<td class="center">
			...
			<c:if test="${ orgSummary['testTakerPercentStudentsTestable'] != null}">
				<c:choose>
					<c:when test="${ orgSummary['testTakerPercentStudentsTestable'] == null or orgSummary['testTakerPercentStudentsTestable'] == 'TBD' or orgSummary['testTakerPercentStudentsTestable'] == '(missing)' }">
		    ...
		    <c:if test="${ orgSummary['createDate'] != null }">
				...
				<c:if test="${ currentOrg.orgTypeCode == 'state' }">
					<button type="button" class="btn btn-xs btn-default export-button" onclick="$('#downloadAllSchoolsCsv').submit();">
						<img src="${pageContext.request.contextPath}/static/images/icons/export.png" class="report-button-image">Export School Detail to .csv
					</button>
				</c:if>
			</c:if>	
		</ui:dataGridHeader>
