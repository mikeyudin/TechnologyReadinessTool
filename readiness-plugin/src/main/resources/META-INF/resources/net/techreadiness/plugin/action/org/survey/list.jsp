...
<html>
<head>
<title><s:text name="surveyQuestions.list.pageTitle" /></title>
</head>
<body>
	<ui:dataView taskFlow="${taskFlow}" filterSelectable="true" title="%{getText('tab.survey.title')}">
	 	<ui:dataViewFilters viewDefType="ORG_DATAGRID">
			<c:if test="${showAllOrgsCheckboxDisplayed}">
				<ui:dataViewFilter name="%{getText('organization.list.searchAll')}" code="showAll" beanName="searchAllSurveyQuestionsHandler" type="checkbox" primary="true"/>
			</c:if>
		...
		<s:form>
			<ui:datagrid itemProvider="${orgItemProvider}" value="surveyQuestionsSearchGrid" var="org" viewDef="${dataGridView}" selectable="true" rowValue="orgId" columnSelectable="true">
				<c:choose>
