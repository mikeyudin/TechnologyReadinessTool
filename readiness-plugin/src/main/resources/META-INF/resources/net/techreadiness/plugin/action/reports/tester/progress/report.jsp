...
<table class="report-layout">
	<tr>
		<td class="left">
			<c:if test="${ dualConsortium }">
				...
				<div class="report-toggle">
					<s:form action="testerProgress" id="consortiumForm">
						<c:choose>
		...
		<td class="center">
			<div class="report-header">Progress Report - Device to Test-Taker Indicators</div>
			...
			<c:choose>
				<c:when test="${ currentOrg.orgTypeName == 'Readiness' }">
					<div class="report-org-text">
						...
						<div class="report-label2">% of Eligible Test-Takers can be Tested on Existing Devices, based on ${requirements} Requirements <img src="${pageContext.request.contextPath}/static/images/moreinfo-reverse.png" title="# Possible Test Starts / # Test Starts Needed"/></div><div style="clear: both;"></div>
					</div>
				</c:when>
		...
		<td class="right">
			...
			<div class="report-legend">
				...
				<div class="legend-row">
					<div class="report-label"><ui:flyout text="TBD" title="Why are some reports showing TBD as a value?" namespace="/reports/tester" action="TBD" reset="true" /></div>
				</div>
...
<s:form action="testerProgress" id="selectOrg">
	<s:hidden name="viewBy"/>
	<s:hidden name="consortium"/>
...
<div style="clear: both;"></div>
<s:form>
	<ui:datagrid itemProvider="${reportItemProvider}" value="reportGrid" var="tester" selectable="false" rowValue="tester.orgCode" columnSelectable="true" >
		<ui:dataGridHeader>
		...
		<s:iterator value="snapshots" var="snapshot">
			<s:set var="snapshotId">testTakerPercentStudentsTestable_${ snapshot.snapshotWindowId }</s:set>
			...
			<s:url var="requirementsUrl" namespace="/reports/tester" action="minRequirements">
			...
			<s:url var="testerAssessmentUrl" namespace="/reports/tester" action="testerAssessment">
				<s:param name="snapshotName" value="#snapshot.name"/>
				<s:param name="snapshotScopeId" value="#snapshot.scope.id"/>
			...
			<ui:dataGridColumn code="${snapshotId}" manageColumnsText="${snapshot.name == 'default' ? 'Current Window' : snapshot.name}" name="${snapshot.name == 'default' ? 'Current Window' : snapshot.name}<a class='entity-name' title='Minimum / Recommended Requirements - ${ executeDate }' href='${ requirementsUrl }'><img style='margin-left: 4px' src='${pageContext.request.contextPath}/static/images/magnifying-glass-white.png'/></a><br>${ executeDate }<a class='#report-desc' title='View of Device to Test-Taker Indicators Report as of ${ executeDate }' href='${ testerAssessmentUrl }'><img style='margin-left: 3px' src='${pageContext.request.contextPath}/static/images/external-link-white.gif'/></a>">
			   <div class="report-datagrid-color-icon">
					<div class="color <s:property value="getPercentColor(tester[#snapshotId])"/>"></div>
					<div class="completion-label"><s:property value="tester[#snapshotId]"/> 
					<s:if test="%{tester[#deviceTBD] > 0}">
