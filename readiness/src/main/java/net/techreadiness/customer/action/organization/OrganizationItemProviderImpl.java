...
public class OrganizationItemProviderImpl extends DataGridItemProviderImpl<Org> implements OrganizationItemProvider {
	...
	public Collection<Org> getPage(DataGrid<Org> grid) {
		Criteria criteria = createCriteria(grid, ShowAllOrganizationsSelectionHandler.SHOW_ALL);
		...
		if (grid.getFilters().get(ShowAllOrganizationsSelectionHandler.SHOW_ALL).isEmpty()) {
			...
			sb.append("WHERE  op.scope_id = :scopeId AND tree.ancestor_org_id = :orgId ");
